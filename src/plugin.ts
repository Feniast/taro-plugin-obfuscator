// Credits: https://github.com/javascript-obfuscator/webpack-obfuscator
"use strict";
...
import { WebpackObfuscatorOptions } from "./types";
...
export class WebpackObfuscatorPlugin {
  static pluginName = "WebpackObfuscator";
  /**
   * @type {string[]}
   */
  ...
  public apply(compiler: Compiler): void {
    ...
    compiler.hooks.compilation.tap(pluginName, (compilation: Compilation) => {
      compilation.hooks.processAssets.tap(
        {
          name: WebpackObfuscatorPlugin.pluginName,
          ...
          compilation.chunks.forEach((chunk) => {
            chunk.files.forEach((fileName: string) => {
              ...
              if (this.options.debug) {
                console.log(
                  `WebpackObfuscatorPlugin: start obfuscating ${fileName}.`
                );
              }
              ...
              if (!asset) {
                if (this.options.debug) {
                  console.warn(
                    `WebpackObfuscatorPlugin: ${fileName} no asset, skip obfuscating.`
                  );
                }
                return;
              }
